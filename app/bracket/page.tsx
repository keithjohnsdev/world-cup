    ...
    <div className="bg-green-950 min-h-screen">
      <div className="max-w-2xl mx-auto px-4 pt-8 pb-24">
        ...
        <div className="text-center mb-12">
          <div className="text-[10px] font-black uppercase tracking-[0.3em] text-green-500 mb-5">World Cup 2026 · Family Bracket Challenge</div>
          <h1 className="font-black uppercase text-white leading-none mb-4" style={{ fontSize: "clamp(3rem, 12vw, 5rem)", letterSpacing: "-0.03em" }}>
            The Rules
          </h1>
          <div className="h-px bg-gradient-to-r from-transparent via-yellow-300/40 to-transparent mb-5" />
          <p className="text-green-300/60 text-sm leading-relaxed">48 nations. 2 phases. One Johnsie champion.</p>
        </div>

        {/* Phases */}
        <div className="mb-12">
          <div className="text-[9px] font-black uppercase tracking-[0.3em] text-green-600 mb-4">How It Works</div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-yellow-300/20 bg-yellow-300/5 p-5">
              <div className="text-yellow-300 text-[9px] font-black uppercase tracking-[0.25em] mb-3">Phase 1</div>
              <div className="text-white font-black text-base leading-tight mb-3">Group Stage</div>
              <div className="text-green-500 text-[10px] font-bold mb-4">Deadline: before the first match</div>
              <ul className="space-y-1.5 text-[11px] text-green-100/80 leading-snug">
                <li>· Rank all 4 teams in each of 12 groups (1st–4th)</li>
                <li>· Name your Champion</li>
              </ul>
            </div>
            <div className="rounded-2xl border border-blue-400/20 bg-blue-400/5 p-5">
              <div className="text-blue-300 text-[9px] font-black uppercase tracking-[0.25em] mb-3">Phase 2</div>
              <div className="text-white font-black text-base leading-tight mb-3">Knockout Bracket</div>
              <div className="text-green-500 text-[10px] font-bold mb-4">Deadline: before Round of 32</div>
              <ul className="space-y-1.5 text-[11px] text-green-100/80 leading-snug">
                <li>· Pick winners through the bracket</li>
        ...
        {/* Group Stage Scoring */}
        <div className="mb-12">
          <div className="text-[9px] font-black uppercase tracking-[0.3em] text-green-600 mb-4">Group Stage Scoring</div>
          <div className="rounded-2xl border border-white/10 overflow-hidden">
            <div className="grid grid-cols-2 divide-x divide-white/10">
              <div className="p-5">
                <div className="text-yellow-300 font-black leading-none mb-1" style={{ fontSize: "3.5rem" }}>2</div>
                <div className="text-[9px] font-black uppercase tracking-[0.2em] text-yellow-300/50 mb-3">pts</div>
                <div className="text-green-100/80 text-[11px] leading-relaxed">Pick finishes top 2 — any top-2 finish counts</div>
              </div>
              <div className="p-5">
                <div className="text-yellow-300 font-black leading-none mb-1" style={{ fontSize: "3.5rem" }}>1</div>
                <div className="text-[9px] font-black uppercase tracking-[0.2em] text-yellow-300/50 mb-3">pt</div>
                <div className="text-green-100/80 text-[11px] leading-relaxed">Pick lands in the exact position you chose (1st–4th)</div>
              </div>
            </div>
            <div className="divide-y divide-white/5 border-t border-white/10 bg-white/5">
              <div className="flex justify-between items-center px-5 py-3">
                <span className="text-green-400/70 text-xs">Max per group</span>
                <span className="text-yellow-300 font-black text-sm">8 pts</span>
              </div>
              <div className="flex justify-between items-center px-5 py-3">
                <span className="text-green-400/70 text-xs">Max group stage total</span>
                <span className="text-yellow-300 font-black text-sm">96 pts</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Knockout Scoring */}
        <div className="mb-12">
          <div className="text-[9px] font-black uppercase tracking-[0.3em] text-green-600 mb-1">Knockout Scoring</div>
          <div className="text-green-600/60 text-[10px] mb-4 uppercase tracking-wide">Every round has the same total available — they only feel bigger</div>
          <div className="space-y-2">
            {[
              { round: "Round of 32", pts: 3, w: "w-[20%]" },
              { round: "Round of 16", pts: 6, w: "w-[35%]" },
              { round: "Quarterfinals", pts: 12, w: "w-[52%]" },
              { round: "Semifinals", pts: 24, w: "w-[70%]" },
              { round: "Final", pts: 48, w: "w-full" },
            ].map(({ round, pts, w }) => (
              <div key={round} className="flex items-center gap-3">
                <div className="w-28 shrink-0 text-green-300/70 text-[11px] text-right">{round}</div>
                <div className="flex-1 h-8 bg-white/5 rounded-lg overflow-hidden">
                  <div className={`h-full ${w} bg-yellow-300/20 border-r-2 border-yellow-300/60 rounded-lg flex items-center justify-end pr-2`}>
                    <span className="text-yellow-300 font-black text-xs tabular-nums">{pts}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* Bonuses */}
        <div className="mb-12">
          <div className="text-[9px] font-black uppercase tracking-[0.3em] text-green-600 mb-4">Bonuses &amp; Mercy</div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-yellow-300/20 bg-yellow-300/5 p-5">
              <div className="text-yellow-300 font-black leading-none mb-1" style={{ fontSize: "2.5rem" }}>+10</div>
              <div className="text-[9px] font-black uppercase tracking-[0.15em] text-yellow-300/50 mb-3">pts</div>
              <div className="text-white font-bold text-xs mb-1">Champion Bonus</div>
              <div className="text-green-100/60 text-[11px]">Your Phase 1 champion pick wins the whole thing</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="text-white/50 font-black leading-none mb-1" style={{ fontSize: "2.5rem" }}>½</div>
              <div className="text-[9px] font-black uppercase tracking-[0.15em] text-white/25 mb-3">pts</div>
              <div className="text-white font-bold text-xs mb-1">Shootout Mercy</div>
              <div className="text-green-100/60 text-[11px]">Your pick loses in penalties — half points instead of zero</div>
            </div>
          </div>
        </div>
        ...
        {/* Awards */}
        <div className="mb-12">
          <div className="text-[9px] font-black uppercase tracking-[0.3em] text-green-600 mb-1">Awards</div>
          <div className="text-green-600/60 text-[10px] mb-6 uppercase tracking-wide">Everyone wins something · announced after the Final</div>
          {([
            { label: "Glory", icon: "★", color: "text-yellow-300", items: [
              ["The Champion", "Most total points overall"],
              ["True Believer", "Named the actual tournament winner before it started"],
              ["Group Stage Guru", "Most points in the group stage"],
              ["The Closer", "Most points in QF and beyond"],
              ["Dark Horse Whisperer", "Named the lowest-ranked team to go deepest"],
              ["Bracket Brainiac", "Highest knockout accuracy percentage"],
              ["Dead Cert", "Correctly predicted all 12 group winners — near impossible"],
            ]},
            { label: "Funny & Consolation", icon: "◎", color: "text-white/40", items: [
              ["Wooden Spoon", "Dead last — with full ceremony and a literal wooden spoon"],
              ["Heartbreak Hotel", "Most picks that lost in penalty shootouts"],
              ["The Human Coin Flip", "Accuracy closest to exactly 50%"],
              ["Help, I've Gone Cross-Eyed", "Team advanced but in the wrong position — so often"],
              ["The Trendsetter", "Most unique picks nobody else made"],
              ["Reverse Oracle", "Most incorrect picks — so reliably wrong you're almost useful"],
            ]},
            { label: "Skill", icon: "◆", color: "text-green-400", items: [
              ["Perfect Round", "Every pick correct in one full round"],
              ["Upset Artist", "Most correctly predicted upsets"],
              ["Crystal Ball", "Most total correct picks across the tournament"],
            ]},
            { label: "Special", icon: "◈", color: "text-blue-400", items: [
              ["Rookie Star", "Best score among players age 10 and under"],
              ["Comeback Kid", "Biggest point swing from bottom half to top"],
              ["The Commentator", "Most enthusiastic trash-talker (voted, not scored)"],
              ["The Hipster", "Most obscure champion pick that went the farthest"],
            ]},
          ] as const).map(({ label, icon, color, items }) => (
            <div key={label} className="mb-6">
              <div className={`text-[9px] font-black uppercase tracking-[0.2em] mb-2 ${color} opacity-70`}>{label}</div>
              <div className="space-y-1.5">
                {items.map(([name, desc]) => (
                  <div key={name} className="flex items-start gap-3 rounded-xl border border-white/8 bg-white/[0.03] px-4 py-2.5">
                    <div className={`${color} text-sm leading-none mt-0.5 shrink-0`}>{icon}</div>
                    <div>
                      <div className="text-white text-xs font-bold">{name}</div>
                      <div className="text-green-400/60 text-[10px] leading-snug mt-0.5">{desc}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Kid Powers */}
        <div className="mb-12">
          <div className="text-[9px] font-black uppercase tracking-[0.3em] text-green-600 mb-1">Kid Powers</div>
          <div className="text-green-600/60 text-[10px] mb-4 uppercase tracking-wide">Age 10 and under · one use each · tell a grown-up first</div>
          <div className="grid grid-cols-2 gap-3">
            {([
              ["Champion Boost", "+20 pts instead of +10 if your champion wins it all"],
              ["Comeback Chip", "Start a round in last place? 3 free bonus points, automatic"],
              ["Heart Pick", "Name your favorite team — every win they get earns you +1 pt"],
              ["Star Power", "Double points on one game. Call it before it starts. One star."],
            ] as const).map(([name, desc]) => (
              <div key={name} className="rounded-2xl border border-yellow-400/20 bg-yellow-400/5 p-4">
                <div className="text-yellow-300 font-black text-[10px] uppercase tracking-[0.1em] mb-2">{name}</div>
                <div className="text-green-100/70 text-[11px] leading-relaxed">{desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* General Rules */}
        <div>
          <div className="text-[9px] font-black uppercase tracking-[0.3em] text-green-600 mb-4">General Rules</div>
          <div className="space-y-1.5">
            {[
              ...
              "Ties are broken by sharing the title — or the fingers game: kids secretly hold up fingers, guess even or odd, reveal. Whoever guesses right wins.",
              "Awards are announced together after the Final.",
            ].map((rule, i) => (
              <div key={i} className="flex gap-3 items-start rounded-xl border border-white/8 bg-white/[0.03] px-4 py-3">
                <span className="text-green-700 font-black text-xs shrink-0 mt-px">{i + 1}.</span>
                <span className="text-green-100/80 text-xs leading-relaxed">{rule}</span>
              </div>
            ))}
            <div className="flex gap-3 items-start rounded-xl border border-yellow-300/25 bg-yellow-300/8 px-4 py-3">
              <span className="text-yellow-300 font-black text-xs shrink-0 mt-px">!</span>
              <span className="text-yellow-300 font-bold text-xs">The Wooden Spoon is non-negotiable and must be accepted with grace.</span>
            </div>
          </div>
        </div>
...
function RulesSection() {
  const [view, setView] = useState<"visual" | "classic">("visual");
  return (
    <>
      <div className="bg-green-950 flex justify-center pt-4 pb-0">
        <div className="flex items-center gap-1 bg-green-900/50 border border-white/10 rounded-full p-1">
          {(["visual", "classic"] as const).map((v) => (
            <button
              key={v}
              onClick={() => setView(v)}
              className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-[0.15em] transition-all cursor-pointer ${
                view === v ? "bg-yellow-300 text-green-950" : "text-green-500 hover:text-green-200"
              }`}
            >
              {v === "visual" ? "Visual" : "Classic"}
            </button>
          ))}
        </div>
      </div>
      {view === "visual" ? <InfographicRulesTab /> : <RulesTab />}
    </>
  );
}
