  ...
  return (
    <header className={`sticky top-0 z-30 ${variantClasses[variant]} ${className}`} style={style}>
      ...
      <div className="relative flex items-center px-4 py-3">
        ...
        {center && (
          <div className="absolute inset-y-0 left-1/2 -translate-x-1/2 hidden xl:flex items-stretch">
            {center}
          </div>
        )}
        <div className="flex items-center gap-2 flex-1 justify-end">
          <div className="hidden xl:flex items-center gap-2">{right}</div>
          <button
            className="xl:hidden flex items-center justify-center w-9 h-9 rounded-lg text-white/60 hover:text-white hover:bg-white/10 transition-all"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-20 xl:hidden" onClick={() => setMobileOpen(false)} />
      )}

      {/* Mobile menu */}
      {mobileOpen && (
        <div className="relative z-30 xl:hidden flex flex-col border-t border-white/10" onClick={() => setMobileOpen(false)}>
          {center && (
